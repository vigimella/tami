...
from cati.image_to_smali import retrieve_smali_from_image
...
def parse_args():
    ...
    group.add_argument('-lf', '--load_file', required=False, type=str, default=None,
                       help='Name of result\'s file to load')
    ...
    group.add_argument('--mode', required=False, type=str, default='cam-gradcam_st1',
                       choices=['IFIM-SSIM', 'DexWave', 'cam-gradcam_st1', 'cam-gradcam_st2', 'cam-gradcam++',
                                ...
                                'cam-scorecam-guided', 'cam-scorecam_fast-guided', 'plot-generator'],
    ...
    group.add_argument('--cg', '--catigradcam', required=False, type=str, default=None,
                       help='Insert path where gradcam for cati has been stored')
    group.add_argument('--c', '--cati', required=False, type=str, default=None,
                       choices=['yes-cati', 'no-cati'],
                       help='Cati tool allows to retrieve the smali code highlighted in the gradcam from the image')
    group.add_argument('--lg', '--legend', required=False, type=str, default=None,
                       help='path where file legend are stored')
...
def _check_args(arguments):
    ...
    if "cam" in arguments.mode:
        if arguments.load_model is None and not arguments.load_file and not arguments.c == 'yes-cati':
